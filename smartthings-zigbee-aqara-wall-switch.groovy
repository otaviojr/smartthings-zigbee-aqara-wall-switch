 ...
 *  Copyright 2021 Otávio Ribeiro (https://otavio.guru)
 *
 *  Version: v1.0.1.1123
 *
...
metadata {
	definition (name: "Zigbee - Aqara Wall Switch", namespace: "otavio.guru", author: "Otávio Ribeiro", filename: "zigbee-aqara-wall-switch", importUrl: "https://raw.githubusercontent.com/otaviojr/smartthings-zigbee-aqara-wall-switch/main/smartthings-zigbee-aqara-wall-switch.groovy") {
        ...
        command "forceRecoveryMode", ["NUMBER"]
        ...
        command "setAsDisconnected", ["NUMBER"]
        command "setAsConnected", ["NUMBER"]
        command "toggle"
...
void createChildDevice(String id) {
    ...
    try {
        ...
        DeviceWrapper cd = addChildDevice("hubitat", driver, "$device.id-$id", [name: "$name $id", label: "$name $id", isComponent: false])
...
void sendZigbeeCommands(ArrayList<String> cmd) {
    ...
    physicalgraph.device.HubAction allActions = new physicalgraph.device.HubAction()
    cmd.each {
            allActions.add(new physicalgraph.device.HubAction(it, physicalgraph.device.Protocol.ZIGBEE))
    }
